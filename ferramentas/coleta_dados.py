...
BAD_URL_MARKERS = (
    ...
    "github.io/loja", "google.com/search", "google.com/url",
    "accounts.google", "localhost", "127.0.0.1",
)


def url_valido(u):
    u = (u or "").strip()
    return bool(u) and u.startswith("http") and not any(
        m in u.lower() for m in BAD_URL_MARKERS
    )
...
def get_link_data(url):
    try:
        ...
        final = url
    estrategias = [
        ("api shopee", lambda: via_api(final)),
        ("playwright", lambda: via_playwright(final or url)),
        ("html direto", lambda: via_html(url)),
        ("jina", lambda: via_jina(final or url)),
    ]
    titulo = None
    for i, (nome, fn) in enumerate(estrategias, 1):
        print(f"  [{i}] {nome}...")
        try:
            t, fp = fn()
        except Exception as e:
            print(f"  {nome} erro: {str(e)[:90]}")
            continue
        if fp:
            return t or titulo, fp
        titulo = t or titulo
    return None, None
                ...
                for ln in f:
                    ...
                    if ln.startswith("http"):
                        if url_valido(ln):
                            ...
                            print(f"url invalida ignorada: {ln[:60]}")
    trocas = {
        n: u for n, u in trocas.items()
        if url_valido(u) or print(f"url invalida ignorada no -{n}: {u[:50]}")
    }
