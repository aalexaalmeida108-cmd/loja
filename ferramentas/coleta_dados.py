...
def rebuild(reg):
    ...
    for r in sorted(reg, key=lambda x: x["num"]):
        if not r.get("img_path") or not os.path.exists(
    ...
    abertas = len(re.findall(r"<div\b", re.sub(r"<!--.*?-->", "", new_html, flags=re.S)))
    fechadas = re.sub(r"<!--.*?-->", "", new_html, flags=re.S).count("</div>")
    print(f"grid reconstruido: {len(cards)} cards | divs {abertas}/{fechadas}")
...
def main():
    ...
    for num, u in trocas.items():
        ...
        for r in reg:
            if r["num"] == num:
                ...
                achou = True
        if not achou:
            reg.append({"num": num, "key": url_key(u), "url": u, "title": "", "img_path": ""})
    ...
    for u in novos:
        if any(r.get("url") == u for r in reg):
            ...
            continue
        reg.append({"num": prox, "key": url_key(u), "url": u, "title": "", "img_path": ""})
    ...
    if alterou or mudanca:
        visiveis = [
            r for r in results if r.get("img_path") and os.path.exists(
                os.path.join(ROOT, r["img_path"])
            )
        ]
        if visiveis or not reg:
            rebuild(visiveis)

    if os.path.isdir(PEDIDOS_DIR):
        for fn in os.listdir(PEDIDOS_DIR):
            os.remove(os.path.join(PEDIDOS_DIR, fn))
    print(f"fim: {len(results)} produto(s) na vitrine")
